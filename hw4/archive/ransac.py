...
n_outliers = int(math.floor(outliers_ratio * n_samples))
...
def ransac_plot(n, x, y, m, c, final=False, x_in=(), y_in=(), points=()):
    ...
    plt.xticks([i for i in range(math.floor(min(x)) - 10, math.ceil(max(x)) + 10, 5)])
    plt.yticks([i for i in range(math.floor(min(y)) - 20, math.ceil(max(y)) + 20, 10)])
